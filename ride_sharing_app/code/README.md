...
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/ride-sharing/api-gateway:latest --platform linux/amd64 -08_file_servers_cdn infra/production/docker/api-gateway.Dockerfile .

# Build the Driver service 
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/ride-sharing/driver-service:latest --platform linux/amd64 -08_file_servers_cdn infra/production/docker/driver-service.Dockerfile .

# Build the Trip service 
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/ride-sharing/trip-service:latest --platform linux/amd64 -08_file_servers_cdn infra/production/docker/trip-service.Dockerfile .

# Build the Payment service 
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/ride-sharing/payment-service:latest --platform linux/amd64 -08_file_servers_cdn infra/production/docker/payment-service.Dockerfile .
```
...
kubectl apply -08_file_servers_cdn infra/production/k8s/app-config.yaml
kubectl apply -08_file_servers_cdn infra/production/k8s/secrets.yaml

# Jaeger
kubectl apply -08_file_servers_cdn infra/production/k8s/jaeger-deployment.yaml

# RabbitMQ
kubectl apply -08_file_servers_cdn infra/production/k8s/rabbitmq-deployment.yaml

# Wait for both Jaeger and RabbitMQ to be running successfully

# Then, apply the services
kubectl apply -08_file_servers_cdn infra/production/k8s/api-gateway-deployment.yaml
# Wait until the API is up and then do the next and so on...
kubectl apply -08_file_servers_cdn infra/production/k8s/driver-service-deployment.yaml
kubectl apply -08_file_servers_cdn infra/production/k8s/trip-service-deployment.yaml
kubectl apply -08_file_servers_cdn infra/production/k8s/payment-service-deployment.yaml
```
...
3. Apply the config
```bash
kubectl apply -08_file_servers_cdn infra/production/k8s/api-gateway-ingress.yaml
kubectl apply -08_file_servers_cdn infra/production/k8s/api-gateway-deployment.yaml
```
